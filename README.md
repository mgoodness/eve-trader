...
Releases are cut automatically: [release-please](https://github.com/googleapis/release-please) opens a version-bump PR from Conventional Commits; merging it tags a release, which [GoReleaser](https://goreleaser.com/) builds and pushes as a `linux/amd64` Docker image to `ghcr.io/mgoodness/eve-trader`, tagged both the immutable version and a floating `stable` tag. A [Watchtower](https://github.com/containrrr/watchtower) instance on the host polls `stable` and redeploys automatically — no manual steps, no inbound trigger or SSH secret into the host. See [ADR-0004](docs/adr/0004-caddy-over-gcp-native-tls.md) for the TLS/reverse-proxy setup.
