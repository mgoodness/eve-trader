...
func TestOpenCreatesLedgerTables(t *testing.T) {
	...
	want := []string{"wallet_transaction", "wallet_journal", "ledger_sync", "contract", "contract_item", "manual_transfer"}
	for _, table := range want {
		var name string
		err := sqlDB.QueryRow(`SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?`, table).Scan(&name)
		if err != nil {
...
func TestOpenAddsLedgerTablesToAV1_1DatabaseWithoutDataLoss(t *testing.T) {
	...
	for _, table := range []string{"wallet_transaction", "wallet_journal", "ledger_sync", "contract", "contract_item", "manual_transfer"} {
		var count int
